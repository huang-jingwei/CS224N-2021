...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        # dependencies_batch = []
        sentences_bacth=unfinished_parses[:batch_size]
        transitions=model.predict(sentences_bacth)
        print("transitions",transitions)

        for i in range(len(sentences_bacth)):
            unfinished_parses.pop()
            # dependencies_batch.append(transitions)
        #     dependencies.append(transitions)
        # for i in range(len(sentences_bacth)):
