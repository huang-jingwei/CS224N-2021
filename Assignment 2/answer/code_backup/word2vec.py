...
def naiveSoftmaxLossAndGradient(
    ...
    dim=outsideVectors.shape[1]
    num=outsideVectors.shape[0]
    centerWordVec = centerWordVec.reshape([dim, 1])
    # 相关性概率向量,向量形状为[num words in vocab,1]
    # y_hat[i]:代表中心词与第i个词的相关性概率,然后进行exp指数计算
    prob = np.exp(np.dot(outsideVectors, centerWordVec))
    y_hat=prob/np.sum(prob)
    # 真实相关性向量,向量形状为[num words in vocab,1]
    # y[i]:代表第i个词是否是中心词的outside词汇
    y=np.zeros([num,1])
    y[outsideWordIdx]=1
    ...
    # Calculate gradient for Center word
    # shape:(num words in vocab, word vector length)
    gradCenterVec=(np.dot(outsideVectors.T,(y_hat-y))).reshape(-1)

    # Calculate gradient for Outside words
    # shape:(word vector length, )
    gradOutsideVecs=(np.dot(y_hat-y,centerWordVec.T)).reshape(num,-1)
...
def negSamplingLossAndGradient(
    ...
    centerWordVec = centerWordVec.reshape([-1, 1])  # [d,1]
    uo = outsideVectors[outsideWordIdx]             # [1,d]
    uk = outsideVectors[negSampleWordIndices]       # [k,d]

    ### Please use your implementation of sigmoid in here.
    prob_uo = sigmoid(np.dot(uo, centerWordVec))     # [1,1]
    prob_neg= sigmoid(-1*np.dot(uk, centerWordVec))  # [k,1]

    # Calculate loss function
    loss=-1*np.log(prob_uo)-np.sum(np.log(prob_neg))

    # Calculate gradient for Center word
    gradCenterVec=-1*(1-prob_uo)*uo+ np.sum(uk*(1-prob_neg).reshape(-1,1),axis=0)
    gradCenterVec=gradCenterVec.reshape(-1) # [d,]

    # Calculate gradient for Outside words
    gradOutsideVecs=np.zeros(shape=outsideVectors.shape) # [n,d]
    gradOutsideVecs[outsideWordIdx] =-1*(1-prob_uo)*centerWordVec.reshape(-1)
    for i,negIdx in enumerate(negSampleWordIndices):
        gradOutsideVecs[negIdx] +=(1-prob_neg[i])*centerWordVec.reshape(-1)
    ### END YOUR CODE
